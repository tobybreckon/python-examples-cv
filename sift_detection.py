...
# Example : SIFT / SURF or ORB feature point detection and matching
...
if (((args.video_file) and (cap.open(str(args.video_file))))
    ...
    if ((int(major) >= 4) and (int(minor) >= 4)):
        ...
        print("Sorry - SIFT unavailable: falling back to ORB")
    ...
    while (keep_processing):
        ...
        if (len(boxes) > 1) and (boxes[0][1] < boxes[1][1]) and (
            ...
            if (h > 0) and (w > 0):
                ...
                cropped_region_with_features = cv2.drawKeypoints(
                    crop, keypoints_cropped_region, None, (255, 0, 0), 4)
        ...
        if (cropped):
            ...
            if (compute_object_position_via_homography):
                ...
                if len(good_matches) > MIN_MATCH_COUNT:
                    ...
                    # construct two sets of points - source (the selected
                    # object/region points), destination (the current frame
                    # points)
                    ...
                    if (transform_image_via_homography):
                        ...
                        # extract the bounding co-ordinates of the
                    ...
                    print(
                        "Not enough matches for found for homography - %d/%d" %
            ...
            draw_params = dict(
                matchColor=(
                    0, 255, 0), singlePointColor=(
                    255, 0, 0), flags=0)
            display_matches = cv2.drawMatches(
                crop,
                keypoints_cropped_region,
                frame,
                keypoints,
